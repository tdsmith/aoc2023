...
, hits AS (
    SELECT
        product(parts.part) AS ratio
    FROM
        parts,
        UNNEST([parts.r-1, parts.r, parts.r+1]) AS hrows(hr),
        UNNEST(generate_series(parts.start-1, parts.end+1)) AS hcols(hc)
        INNER JOIN grid ON (hr = grid.r AND hc = grid.c AND grid.v = '*')
    GROUP BY (grid.r, grid.c)
    HAVING COUNT(parts.part) = 2
)

SELECT SUM(ratio) FROM hits
